...
First I put the data of each playlist in different plots.I noteced that Fleur's playlist does have more danceability. My playlist has a few songs that have very low valence and danceability. Hemke's playlist is a bit of a mix, but has more similarities with my playlist. I added them in a plot together to see the differences better. 
...
```{r}
ggplot(Julie,aes(x=valence,y=danceability,size=energy)) + geom_point() + geom_smooth() +ggtitle("Julie") + expand_limits(x = 0, y = 0)
...
```{r}
ggplot(Hemke,aes(x=valence,y=danceability,size=energy)) + geom_point() + geom_smooth() +ggtitle("Hemke") +expand_limits(x = 0, y = 0)
```
```{r}
ggplot(Fleur,aes(x=valence,y=danceability,size=energy)) + geom_point() + geom_smooth() + ggtitle("Fleur")+expand_limits(x = 0, y = 0)
```
...
I added them in plot together to see the differences better. In this plot you can indeed see that Fleur's playlist lies above my and Hemkes playlist, which means it has more danceability. And that my and Hemke's cross with each other. The features were very similar to what I expected. What i didn't expect were the outliers in the playlists. I have a few songs with low valence, danceability and energy, Fleur has a few songs with low danceability and one with low energy, and Hemke has a few songs with low danceability. I'll go through these song one by one. 
